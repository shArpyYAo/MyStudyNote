...
##### 思路
上面的思路是把两个数组重排，方法是用一个for循环循环两个数组，然后把多出一部分的数组直接接到新的数组后面。
复杂度的o（n），而且是最短的数组长度。也是没有达到题目要求。
思路很简单，自己一开始也有想过，但是也是想到是o（n），所以没继续想。
于是在二分法的路上越走越远，投资太大，铁了心要做出来。。。。。
官方的解答似乎是o（log n + m），找时间去看看
...
那么我们就可以知道，被中位数分割的左半边的最大数始终是小于右半边的最小值的
我们把A数组和B数组分别在它们的任意位置切割，分别是leftA、rightA、leftB、rightB
然后把leftA和leftB放入一个leftPart集合，把rightA和rightB放入rightPart集合
那么按照中位数的作用，如果我们能确认

        1.leftPart的长度等于rightPart的长度
...
注意，A[i]和B[j]并不是各自数组的中位数，只是分割点
我们只要确保这两个条件成立，就可以得出答案了
